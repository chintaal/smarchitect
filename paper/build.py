...
def preprocess_for_springer(tex: str) -> str:
    ...
    springer_front = (
        ...
        r"\artnote{Artifact (the SmArchitect platform) available to reviewers.}"
        r"\maketitle"
    )
    tex = re.sub(
...
def preprocess_for_pandoc(tex: str) -> str:
    ...
    tex = re.sub(
        r"\\figplaceholder\{[^{}]*\}\{(.*?)\}\{(.*?)\}",
        r"\\emph{[Figure placeholder] \1} (generated by: \2)",
        tex,
        flags=re.DOTALL,
    )
...
def compile_latex(tex: Path, jobname: str, label: str) -> bool:
    ...
    # Ensure bibliography is reachable from the aux file directory.
